...
public class VetorTamanhoVariavel<T> implements TDAVetor<T>,Iterable<T> {
    ...
    public void adcionarTodos(T[] novosElementos) {
        if (novosElementos == null || novosElementos.length == 0) {
            return;
        }
        int quantidadeParaAdcionar = novosElementos.length;
        int capacidadeNecessaria = this.tamanho + quantidadeParaAdcionar;
        if (capacidadeNecessaria > this.elementos.length) {
            int novaCapacidade = Math.max(this.elementos.length * 2, capacidadeNecessaria);
            redimensionar(novaCapacidade);
        }
        System.arraycopy(novosElementos, 0, this.elementos, this.tamanho, quantidadeParaAdcionar);
        this.tamanho += quantidadeParaAdcionar;
    }
